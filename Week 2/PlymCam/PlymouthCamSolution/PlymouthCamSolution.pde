...
void setup()
{
  ...
  frameRate(2);
  surface.setTitle("1/2/3 Jump  (M)ask  (D)ifferencing  (P)ublish");
...
void draw()
{
  if (movie.available()) {
    ...
    if (differencing) {
      ...
      if((changedPixelCount>50) && (publishing)) mqtt.publish("plymouth/birds", ""+int(random(1,5)));
...
void keyPressed()
{
  ...
  if (key=='p') publishing=!publishing;
}
